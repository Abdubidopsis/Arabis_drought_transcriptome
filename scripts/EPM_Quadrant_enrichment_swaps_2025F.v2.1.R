...
file_paths_file_EPM_species_1 <- c("~/Desktop/Rhome/creome/assays/tools/protocols/moca/results/Asag_SWX0.8dP7K_Anem/annotation/annotated_motifs_q1q9.csv")
file_paths_file_EPM_species_2 <- c("~/Desktop/Rhome/creome/assays/tools/protocols/moca/results/Anem_SWX0.8dP7K_Asag/annotation/annotated_motifs_q1q9.csv")
...
OUTPUT_FILENAME <- "AnemAsag_k25_SW_comparative_enrichment_summary_species_swap.csv"
...
EPMs_species_1_small <- EPMs_species_1 %>% dplyr::select(gene_id, epm) %>%
  mutate(epm = substr(epm, 1, 17))
colnames(EPMs_species_1_small) <- c("gene_id", "epm_species_1") 

EPMs_species_2_small <- EPMs_species_2 %>% dplyr::select(gene_id, epm) %>%
  mutate(epm = substr(epm, 1, 17))

colnames(EPMs_species_2_small) <- c("gene_id", "epm_species_2")

ortho_DGEs_epm_spec1df <- merge(ortho_DGEs_df %>% dplyr::select(target_gene_id, quadrant), EPMs_species_1_small,
                                by.x = "target_gene_id", by.y = "gene_id")
ortho_DGEs_epm_spec2df <- merge(ortho_DGEs_df %>% dplyr::select(As_orthologs, quadrant), EPMs_species_2_small,
                                by.x = "As_orthologs", by.y = "gene_id")
head(ortho_DGEs_epm_spec1df)
head(k18epms_df)
...
head(ortho_DGEs_epmk25_spec1df)
